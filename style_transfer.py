...
CONTENT_PATH = 'samples/content/temple2.JPG'
STYLE_PATH = 'samples/style/16-style5.png'
...
def saveimg(img, iters):
    if (PIXEL_CLIP == 'True'):
        img = img.clip(0, 255)
    cv2.imwrite('./samples/st_result/out' + str(iters) + '.jpg', img)
...
if (PRESERVE_COLOR=='True'):
    ...
    # show(g_preserve)
    saveimg(g_preserve, 1998) # out333 = final with preseved colors

elapsed_time = time.time()-start_time
print(f"소요 시간: {elapsed_time//60}분 {elapsed_time%60}초")